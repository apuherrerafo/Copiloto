...
export function startSmartCopilotAlerts(): () => void {
  ...
  const run = async () => {
    ...
    // ── Inside eating window: first meal missing + fast ceiling (17h from last 8pm meal) ──
    if (eating && !hasMealAfterWindowOpen) {
      if (elapsed >= 17) {
        const hourSlot = `${date}-h${now.getHours()}`;
        if (tryMark(`fast17-${hourSlot}`)) {
          push(
            `🛑 ${APP_NAME} — Past ${PROTOCOL.fast.maxHours} h fast`,
            `You are past the ${PROTOCOL.fast.maxHours} h mark without a logged meal. Break the fast with something light. Bring this pattern to your clinician — this app is a log, not a prescription.`,
            'hypo-copilot-fast17',
            true,
          );
        }
        ...
        push(
          ...
          `If you have not eaten yet, break your fast soon — crossing ${PROTOCOL.fast.maxHours} h often feels worse with hypothyroidism (energy, mood, stress hormones). Protein-forward first bite if you can.`,
          'hypo-copilot-fastcap',
          true,
        );
      } else if (elapsed >= 16 && elapsed < 17 && tryMark(`fast16band-${date}`)) {
        push(
    ...
    if (eating && minToEatClose !== null && minToEatClose <= 45 && minToEatClose >= 3 && tryMark(`eatclose-${date}`)) {
      push(
        ...
        `About ${minToEatClose} min until 8:00 PM. Closing on time keeps your overnight fast aligned with your plan — helpful for sleep and next-day levothyroxine timing.`,
        'hypo-copilot-eatclose',
      );
    }

    // ── Before noon: morning awareness (fast still counting from last night) ──
    const h = now.getHours();
    if (!eating && h >= 10 && h < 12 && elapsed >= 14 && elapsed < 15.5 && tryMark(`preopen-${date}`)) {
      push(
        `🌤️ ${APP_NAME} — Eating window soon`,
        `Your eating window opens at 12:00. Hydrate and think protein + fat first after the pill window — steady fuel beats a sugar spike for thyroid-friendly days.`,
